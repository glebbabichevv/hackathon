  ...
  return (
    <div className="rounded-2xl border border-[#1a3050] bg-[#0a1628] p-4">
      ...
      <div className="flex gap-2">
          ...
          className={`flex-1 flex items-center gap-2 p-3 rounded-xl border transition-all duration-200 ${
            provider === 'claude'
              ...
              : hasAnthropicKey
                ...
                : 'border-[#1a3050] opacity-40 cursor-not-allowed'
          }`}
        >
          ...
          <div className="text-left">
            ...
            <p className="text-[10px] text-slate-500">
              {hasAnthropicKey ? 'Anthropic API' : 'Нет ключа'}
            </p>
          </div>
          {provider === 'claude' && (
        ...
        >
          ...
          <div className="text-left">
            ...
            <p className="text-[10px] text-slate-500">
              {ollamaStatus.available ? `${ollamaStatus.models.length} моделей` : 'Не запущен'}
            </p>
          </div>
      ...
      {provider === 'ollama' && ollamaStatus.available && ollamaStatus.models.length > 0 && (
        <div className="mt-3">
          <p className="text-[10px] text-slate-500 mb-1.5">Модель:</p>
      ...
      {!ollamaStatus.available && (
        <div className="mt-3 rounded-lg border border-yellow-500/20 bg-yellow-500/5 px-3 py-2">
          ...
          <p className="text-[10px] text-slate-500 mt-0.5">
            <code className="text-yellow-300">ollama serve</code>
          </p>
        </div>
      )}
    </div>
